...
- The task might be to predict customer behaviour but we should always remember the end goal is to drive business growth. 
- This should be kept in mind during the analysis. 
- here we want to predict whether the customer will attend the class or not but the end goal should be how can we get more customers to attend the class?
...
i'm gonna go ahead and import a function that will provide us with plots for this 


```{python}
#| fig-align:center
from utils import get_relations
get_relations(X_train, y=y_train, which="numerics")
```
...
## 6.2 Pipeline
- Here we have basically defined a function that uses the pipeline we have created to train the models we have defined
...
```{python}
#| fig-align: center
scores = get_scores(model_dict = models, X = X_train, y = y_train)
scores.plot.bar()
```
